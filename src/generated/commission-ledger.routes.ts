...
/**
 * Auto-generated CommissionLedger Routes
 *
...
import type { CommissionLedgerHooks } from "./commission-ledger.hooks"
...
let hooks: CommissionLedgerHooks = {}
...
/**
 * Set hooks for CommissionLedger routes
 */
export function setCommissionLedgerHooks(h: CommissionLedgerHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["purchaseId", "sellerAddress", "amount", "txHash", "status", "createdAt"] as const
...
/**
 * Create CommissionLedger CRUD routes
 */
export function createCommissionLedgerRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[CommissionLedger] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.commissionLedger.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "CommissionLedger not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[CommissionLedger] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.commissionLedger.create({
        data: picked.data,
      })
      ...
      console.error("[CommissionLedger] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.commissionLedger.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[CommissionLedger] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[CommissionLedger] Delete error:", error)
