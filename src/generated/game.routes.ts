...
/**
 * Set hooks for Game routes
 */
export function setGameHooks(h: GameHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["title", "description", "genre", "price", "rating", "players", "imageUrl", "featured", "isNew", "createdAt"] as const
const RELATION_FIELDS = ["purchases"] as const
...
/**
 * Create Game CRUD routes
 */
export function createGameRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.game.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Game not found" } }, 404)
      }
