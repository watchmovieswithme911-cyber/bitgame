...
/**
 * Auto-generated GamePurchase Routes
 *
...
import type { GamePurchaseHooks } from "./game-purchase.hooks"
...
/**
 * Set hooks for GamePurchase routes
 */
export function setGamePurchaseHooks(h: GamePurchaseHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["gameId", "buyerAddress", "sellerAddress", "amount", "commissionRate", "commissionAmount", "sellerAmount", "txHash", "status", "confirmations", "createdAt", "confirmedAt"] as const
const RELATION_FIELDS = ["game"] as const
...
/**
 * Create GamePurchase CRUD routes
 */
export function createGamePurchaseRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.gamePurchase.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[GamePurchase] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.gamePurchase.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "GamePurchase not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[GamePurchase] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.gamePurchase.create({
        data: picked.data,
      })
      ...
      console.error("[GamePurchase] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.gamePurchase.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[GamePurchase] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[GamePurchase] Delete error:", error)
